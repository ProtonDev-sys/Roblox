...
local Identifier = {52,54,50,48,100,52,49,51,102,97,50,52}
local Nonce = 0
...
local function Verify(Key)
  local Response = game:HttpGet(("https://mikecash.co/api/keyverify?key=%s&project=4620d413fa24&nonce=%s"):format(Key, tostring(Nonce)))
  local Decode = HttpService:JSONDecode(Response)
  Verified = Decode.success and Decode.nonce == ShaNonce
end

if isfile("remember_me_4620d413fa24.txt") then
  Verify(readfile("remember_me_4620d413fa24.txt"))
end

Iris:Connect(function()
  if not Verified then
    Iris.Window({"https://mikecash.co/key/Proton", [Iris.Args.Window.NoClose] = true, [Iris.Args.Window.NoResize] = true, [Iris.Args.Window.NoScrollbar] = true, [Iris.Args.Window.NoCollapse] = true}, {size = Iris.State(Vector2.new(175, 75))}) do
        ...
        if Iris.Button({"Check key"}).clicked then
          if CanPress then
            task.spawn(function()
              ...
              if Verified then
                writefile("remember_me_4620d413fa24.txt", Key)
              end
              task.wait(2)
              Status = "Check key"
              CanPress = true
