...
local function getAllAttackableOres(ignoreStone, range, enforceDepth)
    ...
    local function getAllAttackableOres(ignoreStone, range, enforceDepth)
        ...
        if ignoreStone and #attackableOres == 0 and range == math.huge and enforceDepth then
            local lowestY = math.huge
            local lowestOre = nil
            for _, chunk in ipairs(workspace.MineChunks:GetChildren()) do 
                for _, ore in ipairs(chunk:GetChildren()) do
    ...
    local ores = {}
    local function getNextMiningOre(bool, distance)
        local found = false
        ...
        if not found  then
            ...
            return getNextMiningOre(bool, distance)
        end
        return ores[1]
    end
