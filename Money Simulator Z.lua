...
local function getAllAttackableOres(ignoreStone, range, enforceDepth)
        ...
        if ignoreStone then
            if #attackableOres == 0 then
                return getAllAttackableOres(false, math.huge, true)
            end
    ...
    local function getAllAttackableOres(ignoreStone, range, enforceDepth)
        ...
        -- If we're ignoring stone and found no attackable ores, change the parameters and try again
        if ignoreStone and #attackableOres == 0 then
    ...
    local function getNextMiningOre(bool, distance, rec)
        ...
        elseif not found and rec then
            ores = getAllAttackableOres(bool, distance, false)
            table.sort(ores, function(a,b)
                local priorityA = oreList[a.Name] or 0
                local priorityB = oreList[b.Name] or 0
                return priorityA > priorityB
