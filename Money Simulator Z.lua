...
do
    ...
    local function getAllAttackableOres(ignoreStone)
        ...
        for _,v in next, workspace.MineChunks:GetChildren() do 
            for _,v in next, v:GetChildren() do
                if v:IsA("BasePart") and v.Name ~= "Bedrock" then
    ...
    local ores = {}
    local function getNextMiningOre(bool, rec)
        local found = false
    ...
    QuarryLeftBox:AddToggle('AutoMineQuarryLegit', {
        ...
        Callback = function(Value)
            while Toggles.AutoMineQuarryLegit.Value do
                ...
                if ore and (ore.Position - player.Character.HumanoidRootPart.Position).Magnitude < 10 and not getgenv().isMiningQuarry then
                    local highlight = Instance.new("Highlight")
                    highlight.FillColor = Color3.new(0,255,0)
                    highlight.Parent = ore
                    highlight.Enabled = true
                    repeat
                        game:GetService("ReplicatedStorage").Events.Mine:FireServer(ore)
                        wait()
                    until not ore.Parent or not Toggles.AutoMineQuarryLegit.Value or getgenv().isMiningQuarry
                    table.remove(ores, table.find(ores, ore))
                    highlight:Destroy()
